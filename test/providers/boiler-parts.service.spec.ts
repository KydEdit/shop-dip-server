...
describe('Auth Service', () => {
  ...
  it('should find by name', async () => {
    const part = await boilerPartsService.findOneByName('Astrum thesaurus.');
  ...
  it('should find by search string', async () => {
    const parts = await boilerPartsService.searchByString('rus');

    expect(parts.rows.length).toBeLessThanOrEqual(20);

    parts.rows.forEach((item) => {
      expect(item.name.toLowerCase()).toContain('rus');
